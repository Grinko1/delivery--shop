...
const CreateProductModal = () => {
  ...
  const formatTypes = useMemo(() => types.map(({ name, id }) => ({ title: name, id })), [types]);
  ...
  const addDevice = () => {
    ...
    //       .then((data) => {
    //         alert(`Успешно создан продукт ${data.name}`);
    //       })
    //       .catch((error) => {
  ...
  return (
    <div className={styles.createProduct}>
      ...
      <Input
        ...
        value={releaseForm}
        onChange={(event) => {
          setReleaseForm(event.target.value);
        }}
      />
      <Input
        ...
        onChange={(event) => {
          setMethodOfApplication(event.target.value);
        }}
      />
      <Input
